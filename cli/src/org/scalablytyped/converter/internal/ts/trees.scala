...
/**
 * Base trait for all TypeScript AST (Abstract Syntax Tree) nodes.
...
/**
 * Base class for TypeScript literal values.
 * In TypeScript: `42`, `"hello"`, `true`, `false`
 * Represents compile-time constant values.
 */
...
/**
 * Base trait for TypeScript identifiers (names).
 ...
 * In TypeScript: when you `import { something } from "module"`, `something` becomes a TsIdentImport
...
final case class TsIdentModule(scopeOpt: Option[String], fragments: List[String]) extends TsIdent {
  ...
  /**
   * Constructs the full module name.
   * Examples: "lodash", "@types/node", "@scope/package/submodule"
   */
...
/**
 * Represents a TypeScript library/package identifier.
...
object TsIdentLibrary {
  ...
  /**
   * Parses a string into a library identifier.
   * Handles both simple names and scoped packages.
   */
...
final case class TsTypeObject(
                               ...
                               members: IArray[TsMember]
                             ) extends TsType with HasClassMembers
...
object TsTypeIntersect {
   ...
   * Merges multiple object types into a single object type for better readability.
   */
...
object TsTypeUnion {
   ...
   * Returns never for empty unions, single type for one-element unions.
   */
  def simplified(types: IArray[TsType]): TsType =
...
/**
 * Base trait for all TypeScript class and interface members.
